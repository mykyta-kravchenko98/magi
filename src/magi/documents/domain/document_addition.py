...
from magi.documents.domain.value_objects import ProcessingFailure, SourceFileMetadata
...
class DocumentAddition:
    ...
    def _validate_state(self) -> None:
        ...
        if self.status is DocumentAdditionStatus.ACCEPTED:
            if self.source_object_reference is not None or has_result or has_failure:
        ...
        if self.status is DocumentAdditionStatus.PROCESSING:
            if not self.source_object_reference or has_result or has_failure:
        ...
        if self.failure is None or has_result:
            raise DomainRuleViolation(
                "a failed addition requires a code and cannot contain a result"
            )
