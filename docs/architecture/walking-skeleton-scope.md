...
7. Compose same-page numbered part/chapter labels with their titles, classify content roles, and select `body` and `front_matter` for indexing; retain but exclude `table_of_contents`, `header_footer`, and `footnote`.
